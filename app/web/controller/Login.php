...
class Login
{
    ...
    public function system()
    {
        ...
        $docSetting = empty($setting['doc']) ? [] : json_decode($setting['doc'], true);
        $searchSetting = empty($setting['search']) ? [] : json_decode($setting['search'], true);
    ...
    public function h5()
    {
        ...
        if (!$code) {
            ...
            try {
                ...
                header('location:/h5/#/pages/login/token?token=' . $token . '&from=' . $fromUrl);
                exit;
            } catch (\Exception $e) {
