...
class Video extends Base
{
    ...
    public function getHistoryMsg()
    {
        ...
        if (!in_array($platform, ['pika', 'runway', 'stable', 'sora'])) {
        ...
        try {
            ...
            if ($platform == 'pika') {
                if ($channel == 'pikapikapika') {
                    $pikaSDK = $this->newPikaSDK($videoSetting['pika']);
                    if (!$pikaSDK) {
                        $this->setVideoFail($video_id, 'key已用尽，请等待平台处理');
                    } else {
                        $options['prompt'] = $promptEn;
                        if ($action == 'generate') {
                            $result = $pikaSDK->generate($options);
                        } elseif ($action == 'edit') {
                            unset($options['image']);
    ...
    public function getVideoResult()
    {
        ...
        if ($video['state'] != 2 && !empty($video['job_id'])) {
     ...
     */
    private function newPikaSDK($pikaSetting = null)
    {
        if (!$pikaSetting) {
            $videoSetting = getSystemSetting(self::$site_id, 'video');
            $pikaSetting = $videoSetting['pika'] ?? [];
    ...
    public function getVideoSetting()
    {
        ...
        if (!$setting['pika']['is_open'] && !$setting['runway']['is_open'] && !$setting['stable']['is_open'] && !$setting['sora']['is_open']) {
