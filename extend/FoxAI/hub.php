...
class hub
{
    ...
    public function sendText()
    {
        ...
        $respFunc = function ($ch, $data) use (&$respTemp, $replyCallback, $errorCallback) {
            ...
            if (in_array(self::$ai, ['openai3', 'diy32', 'diy33', 'openai4', 'diy42', 'diy43', 'azure_openai3', 'azure_openai4', 'wenxin', 'wenxin4', 'zhipu', 'zhipu4', 'lxai', 'tongyi', 'skychat', 'claude2', 'kimi', 'baichuan', 'deepseek', 'shangtang', 'doubao'])) {
    ...
    public function search($model, $type, $content)
    {
        ...
        $respTemp = '';
        $respFunc = function ($ch, $data) use (&$respTemp, $replyCallback, $errorCallback) {
            // file_put_contents('./search2.txt', $data . '|', 8);
            $dataLength = strlen($data);
    ...
    private function makeQuestions()
    {
        ...
        if (self::$ai == 'gemini') {
            ...
            // 前置指令
            if (self::$systemPrompt) {
                if (in_array(self::$ai, ['openai3', 'diy32', 'diy33', 'openai4', 'diy42', 'diy43', 'azure_openai3', 'azure_openai4', 'zhipu', 'zhipu4', 'lxai', 'tongyi', 'skychat', 'kimi', 'deepseek', 'shangtang', 'doubao'])) {
                    $questions[] = [
                        'role' => 'system',
    ...
    private function parseError($data)
    {
        ...
        if (in_array(self::$ai, ['openai3', 'diy32', 'diy33', 'openai4', 'diy42', 'diy43', 'azure_openai3', 'azure_openai4', 'zhipu', 'zhipu4', 'lxai', 'tongyi', 'deepseek', 'shangtang'])) {
            if (isset($data->error)) {
        ...
        } elseif (in_array(self::$ai, ['claude2', 'kimi'])) {
            if (isset($data->error)) {
                $errorMsg = $data->error->message;
            }
        } elseif (self::$ai == 'baichuan') {
            if (isset($data->error)) {
                $errorMsg = $data->error->message;
            } else {
                $errorMsg = '';
            }
        } elseif (self::$ai == 'doubao') {
            if (isset($data->error)) {
                $errorMsg = $data->error->message;
            }
